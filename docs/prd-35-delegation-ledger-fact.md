...
22. `gbfleet until` calls `delegate` for the seed of the next free cluster, with the brief's lane and tier, BEFORE `mint_enrolment` for the seat that child will hold; `--tier` overrides the tier. A seat minted when no cluster carries an item makes no `delegate` call, and a refused `delegate` does not stop the spawn.
...
- **The unattended loop's delegation is a prediction.** `gbfleet until` delegates the seed of the next free cluster, but the child claims through the divvy, which may hand it a different cluster by the time it registers. The delegation then reads `expired, nothing claimed` while the child is working something else. That is true — nothing claimed that item on that delegation — and it is visible, which is better than the alternative of the loop writing nothing. A later PRD may let the seat carry the delegated item so the child claims it by id.
