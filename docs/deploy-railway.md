...
| `TRUSTED_HOPS=2` | Two proxies in front: Railway's edge, then this stack's nginx. The app reads two hops from the right of `X-Forwarded-For` — what the edge observed. **Not `1`**: the last hop is nginx's view of the edge, which would put every hosted caller in one bucket (GRPH-553). |
| `TRUSTED_PROXY=true` | Deprecated, superseded by `TRUSTED_HOPS`. Reads the first hop; correct here only while the edge overwrites rather than appends. |
