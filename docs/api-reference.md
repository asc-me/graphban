...
| GET | `/api/fleet/profile` | JWT | The caller's harness preferences (PRD-37 / PRD-41 D20): default and per-project override side by side, plus which one is in force. Includes `budget_tokens` (soft per-sign-off target, null = none). `project_id` optional |
| PUT | `/api/fleet/profile` | JWT | Write the caller's own profile — `defaults` (ordered allowlist), `weights` (0–1 per axis), `excludes`, `budget_tokens`; `project_id` makes it an override. 422 names the field it refuses |
