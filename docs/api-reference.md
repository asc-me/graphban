...
| GET | `/api/harness` | JWT | The Harness page's whole read (PRD-38 PR 2, re-keyed PRD-41 S1): family rollups of vendor x model x version x capability x size band, each with nested leaves (grey under the floor), `n`, `below_floor`, sampling counts, skew badge, median seconds and a cost proxy that states when it will not compare. Serves the capability enum and family map (`capability_set`) and the derivation's coverage. `window_days` (default 90); `versions=current` (default, newest binary per vendor+model plus the previous one across a version change) or `all` |
