...
| D10 | Config precedence: flag, then environment, then `~/.graphban/gb.json` — and the two tools share a DIRECTORY, not a file | **Amended by the grill.** The draft had `gban` reading `graphban`'s `config.json` and ignoring the keys it did not know. That is true and insufficient: the risk is not `gban` misreading a database password, it is the password living in a file that now has a second consumer and a second reason to be copied. So `gban` reads `~/.graphban/gb.json` (`url`, `project`) and `~/.graphban/session.json` (the refresh token), and **never opens `config.json` at all** — a property a test asserts by filename rather than a promise about which keys get parsed. `graphban` runs in a container against the database; `gban` runs on a laptop against HTTP; the credential that must not cross that line lives in its own file, so copying a config never carries it. `gban login` writes `gb.json` if it is absent. |
