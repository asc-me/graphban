...
| `delegate` | `id`, `lane`, `tier`, `note`, `seat`, `wave`, `agent_id` | Record that you are handing an item to a child you are about to spawn (PRD-35). **Claims nothing, spawns nothing.** With `seat: true` (PRD-36) it also mints a worker seat **bound to the item** and returns its `enrolment_code`: registering on that seat claims the item server-side. Needs the mint gate (planner or all-in-one), and is refused when the item's areas are reserved by someone else, naming the holder. `lane` and `tier` are required with no default — the brief suggests, you commit. Links when a declared child (`parent_agent_id`, or a seat you minted) claims the item; anyone else's claim closes it as `superseded`. Your own open one is withdrawn by re-delegating; another agent's is refused until it expires. Refused on a blocked, held or bounce-pinned item |
...
| `collision_clusters` | `project_id`, `status`, `prd_id`, `holds` | Partition takeable work into clusters that provably share no touch-areas; `predicted` marks lower-confidence grouping; `withheld` lists what the pool declined and why (a dependency not done, or handed back until a planner delegates it) |
| `claim_cluster` | `agent_id`, `max_items`, `lease_seconds`, `wait_seconds` | Claim a whole non-colliding cluster and reserve its areas, checked against in-flight work; a refusal names who holds what and what was `withheld` |
...
| `release_item` | `id`, `agent_id`, `to_status`, `reason` | Return a claimed item to the queue; `reason` is noted on the item, and the hand-back is counted |
...
| `get_item_details` | `id` | Every field the item reads share (`project_id`, `touchpoints`, `prd_id`, `prd_section`, authorship, bounce, hold — GRPH-821: built on the one renderer) + `description`, `blocker`, `pr`, `branch` + **`links`** (every edge touching the item, both directions, endpoints rendered) + linked shards + linked requests + **`brief`** (PRD-35): the delegation packet — summary, touchpoints, blockers, task class, lessons, a `lane` and `tier` **suggestion with its `basis`**, `previous` / `attempts` history, `text` to paste into a spawn (which carries no suggestion), and the same **`profile`** / **`policy`** pair `fleet_status` carries (PRD-37 D9) — never in `text` |
...
**The pool is one function, and it withholds** (GRPH-783). `claim_cluster`, `claim_next` and
`collision_clusters` all draw from `items.claim_pool`: claimable, and — for `backlog`/`next`
work — with every dependency done. What the pool declines comes back as `withheld`, one row
per item with `why`: `dependency` (and `blocked_by`), or `released` (and the count). A
`claim_cluster` refusal names them in `reason` too, because "nothing ready to claim" and
"three items, each waiting on something that is not you" are different answers and a worker
seat spinning on the second reads the first as done. The `released` hold is the count of
worker hand-backs: at two, an item is parked out of the pool until a planner `delegate`s it,
which resets the count. It was measured before it was built — one item released three times
over five days, each time with a reason in its evidence, and nothing reading it. `release_item`
takes a `reason` for the same purpose; it lands on the item as a note.
