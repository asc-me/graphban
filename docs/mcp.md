...
| `describe_code` | `nodes`, `edges`, `prune`, `project_id` | **Record code structure** — upsert code nodes and typed edges. Idempotent by path; re-describe on change. `kind` is **`module`** = a package/directory, **`file`** = one source file, **`symbol`** = `path::name` inside a file; a kind contradicting its path is stored as the path implies and reported in `kind_corrections` |
