...
| `credentials` | `id` | One LLM provider credential, owned by the deployment (`org_id` NULL) or by an org. Keyed by ROW, so two keys for the same provider are two rows (PRD-25 D-a). **`serves`** (`chat`/`embed`/`extract`/`decide`) is what the row may be pointed at (PRD-45 S2) |
| `deployment_config` | `scope` | The default / fallback / embedding / **decider** credential for one scope (`decider_credential_id`). `scope` is `''` for the deployment itself, an org id under hosted multi-tenancy. Migration `0131` |
