...
| `gbagent` | `gbagent 0.4.0` | **exactly `0.4.0`** — a pin, not a range | `--mcp-config <path>` | `--instruction-file <path>` | **no** — private temp file |
...
`test_adapters.py` runs the real binary. When the package version changes, the `0.4.0` above
