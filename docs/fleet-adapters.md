...
| `gbagent` | `gbagent 0.8.2` | **exactly `0.8.2`** — a pin, not a range | `--mcp-config <path>` | `--instruction-file <path>` | **no** — private temp file |
...
`test_adapters.py` runs the real binary. When the package version changes, the `0.8.2` above
