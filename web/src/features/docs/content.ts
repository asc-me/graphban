...
const CONTENT: Record<string, DocEntry> = {
  ...
  "/live": {
    ...
    sections: [
      { num: 1, h: "Grouped by human", b: "Each person is one row of agents. Filter by user; Unattributed is a real bucket when a key has no user. All includes them. Offline agents are collapsed under headers that state their counts — one for agents whose only hold is an orphaned branch, one for agents holding nothing — and one click shows either. An offline agent that still holds work (a holding, a lease, an unclaimed delegation) stays in view, faded, because that is the row worth seeing." },
