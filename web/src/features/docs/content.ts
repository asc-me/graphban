...
const CONTENT: Record<string, DocEntry> = {
  ...
  "/mcp-tools": {
    ...
    sections: [
      ...
      { num: 2, h: "Authenticate agents", b: "Mint an API key in Settings → API keys, then call the endpoint with it. A seat on Fleet is a role for one session, not a key. Every call is metered here." },
  ...
  "/settings/project/api-keys": {
    ...
    sections: [
      { num: 1, h: "Three kinds", b: "This page mints Graphban keys. LLM credentials (what the box runs on) live on AI Providers — Looking for LLM credentials? is that question. An agent key talks to MCP — read and write on items, memory, and code. A link key pushes a code graph from a local box into exactly one project and nothing else — same object Sync / Link mints. A gate key attests that work was checked so an item may reach done: for CI or a reviewer, never for the agent doing the work. Gate is minted with read+write+gate; gate alone cannot attest." },
      ...
      { num: 5, h: "Minted with", b: "After the banner, each row opens what that key was minted with: scopes, advertised MCP tool groups, project/global, expiry. Tools are listed for agent keys only — a link key and a gate key do not call MCP, so a tools line on them would name a capability they do not have. core is always named; no extra tiers is core-only, not missing." },
