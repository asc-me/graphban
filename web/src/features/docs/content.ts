...
const CONTENT: Record<string, DocEntry> = {
  ...
  "/settings/project/api-keys": {
    ...
    sections: [
      ...
      { num: 5, h: "Minted with", b: "Each row shows what that key can do: its scopes, and — for agent keys only — the advertised MCP tool groups and how many tools its tools/list carries. The count is the server's, from the same manifest the key is shipped, so a key that shows 34 tools has no tier, whatever was intended at mint. Minted with opens the rest: project/global and expiry. A link key and a gate key do not call MCP, so no tools line on them. core is always named; no extra tiers is core-only, not missing." },
