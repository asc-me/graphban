...
def render_cursor_rules(ledger_loop: str) -> str:
    """`.cursor/rules/agentledger.mdc` — primes an editor agent on the ledger loop (GRPH-147).

    **`.mdc`, not `.md`.** Cursor's rules system ignores plain `.md` files in this directory
    entirely, so the wrong extension produces a file that looks installed, reviews fine, and
    does nothing. That is the whole failure mode this file has to avoid.

    **`alwaysApply: true`**, which is the one frontmatter combination that fits. Cursor's
    rule anatomy: `alwaysApply: true` includes the rule unconditionally; `false` plus `globs`
    auto-attaches on matching files; `false` plus `description` lets the agent pull it in when
    it judges it relevant; neither means the rule only arrives on an explicit @-mention. A
    loop that tells an agent to CLAIM WORK BEFORE STARTING cannot be conditional on which
    file happens to be open — by the time a glob matches, the agent has already started.

    **So it is kept short, deliberately.** An always-applied rule rides on every request, the
    same running cost as the MCP manifest, which has a token budget for exactly this reason.
    It carries the loop and points at AGENTS.md for everything else rather than restating the
    invariants — the item asked for a thin pointer, and a second copy of the invariants would
    drift from the first.

    Generated rather than hand-written so it cannot drift from AGENTS.md at all: the body IS
    the `## Ledger loop` section, verbatim, and `--check` fails if the committed file has
    fallen behind.
    """
    return "\n".join([
        "---",
...
def render_files() -> dict[str, str]:
    ...
    ledger_loop = extract_section(agents_md, "Ledger loop")
    if not ledger_loop:
        raise SystemExit("could not extract '## Ledger loop' from AGENTS.md")
