...
# ---- PRD-38 walk finding: the handoff is the branch, so the work has to be on it ------------

def test_every_builder_instruction_tells_the_child_to_commit():
    """The finding, as a test. Six children across two harnesses wrote the right text on the
    deployed instance and committed none of it, because nothing had ever told them to — the
    instruction names the branch and says to move the item to review, and editing files then
    doing that follows it to the letter.

    Sabotage: drop the clause from either builder template and this fails.
    """
    from gbfleet import seat as seat_mod

    for name, tmpl in (("INSTRUCTION", seat_mod.INSTRUCTION),
                       ("BOUND_INSTRUCTION", seat_mod.BOUND_INSTRUCTION)):
        rendered = tmpl.format(code="ABC-123", worktree="/w", branch="gb/x", item="GRPH-1")
        assert "COMMIT" in rendered, f"{name} never tells the child to commit"
        assert "gb/x" in rendered, f"{name} does not name the branch to commit on"
        # BOTH halves of the reason: an uncommitted change is invisible to the reviewer AND
        # to `touchpoints.measured`, which diffs against the recorded base. A clause that
        # kept only the first would leave the ledger's "what did this attempt change" answer
        # silently empty, which is the same defect wearing a different hat.
        assert "invisible to review" in rendered, (
            f"{name} says to commit without saying why, which is the half that makes it stick")
        assert "touchpoint measurement" in rendered, (
            f"{name} omits that an uncommitted change also measures as having touched nothing")


def test_a_reviewer_is_not_told_to_commit():
    """A reviewer builds nothing. Telling it to commit would be an instruction it cannot
    follow, and instructions a child cannot follow are how the useful ones get ignored."""
    from gbfleet import seat as seat_mod

    rendered = seat_mod.REVIEWER_INSTRUCTION.format(code="ABC-123", worktree="/w", branch="b")
    assert "COMMIT" not in rendered


def test_the_commit_clause_reaches_a_bound_child_through_instruction_for():
    """The template is not the delivery. `instruction_for` picks and formats, and the walk's
    children were bound ones."""
    from pathlib import Path

    from gbfleet.seat import Seat, instruction_for

    text = instruction_for(Seat(code="ABC-123", server_url="http://x", api_key="k",
                                item="GRPH-7"), Path("/w"), "gb/walk-1")
    assert "COMMIT your work on 'gb/walk-1'" in text
    # And the enrolment code is still where every vendor's parser looks for it.
    from gbagent import cli

    assert cli.enrolment_code(text) == "ABC-123"
